...
import Element exposing (Column, Element, rgb, rgba)
...
import Time exposing (Month(..), now, toYear, utc)
import Url
import Url.Builder
...
init flags url key =
    ( { key = key
      ...
      , pdpList = Nothing
      , pdpSelect = Nothing
      , partB = Just "$164.90" -- hardcoded; MUST MANUALLY UPDATE
      , recentError = ""
      , today = Nothing
...
update msg model =
    case msg of
        ...
        RequestPDP ->
            -- runs when you submit, go to page 2
            ...
            case String.toInt str of
                Just i ->
                    if String.length str == 5 then
                        ...
                        ( validateModel
                            { model
                                | zip = ValidInt (Just str) False "Zip must be 5 digits long"
                            }
                        , Cmd.none
                        )
        ...
        PlanResponse rmsg ->
            case rmsg of
                Ok response ->
                    let
                        newRows =
                            List.indexedMap planToRow response
        ...
        PDPResponse rmsg ->
            case rmsg of
                Ok response ->
                    let
                        pr_sort =
                            List.sortBy .plan response

                        prs =
                            List.head <| List.filter (pdpYearFilter model) pr_sort
                    in
                    ( { model
                        | pdpList = Just pr_sort
                        , pdpSelect = prs
                      }
                    , Cmd.none
                    )
        ...
        GotTime timenow ->
            let
                ...
                firstChoice =
                    List.drop 1 choices_ |> List.head
...
navButton clist msg tx =
    let
        classList =
            List.map (\a -> class a) <| [ "two columns" ] ++ clist
    in
    div classList
        [ button
...
renderOutput model =
    let
        tl =
            List.map2 Tuple.pair [ model.planG, model.planN, model.planF ] [ G, N, F ]

        tlf =
            List.filter (\a -> Tuple.first a) tl

        pl =
            List.map Tuple.second tlf

        tables =
            List.map (outputTable model) pl
    in
...
personalInfo model =
    let
        ...
        row2 =
            ageText ++ " yrs" ++ "   |   " ++ zipText ++ "   |   " ++ genderString model.gender ++ "   |   " ++ "Discount Applied: " ++ dsc
        ...
        dentalLink =
            "https://www.securitylife.com/personal-plans?agnt=010U3815"
...
makeEnrollRow ls =
    let
        ll =
            List.sortBy .displayName <| List.filter .selected ls
    ...
    case model.tableRows of
        Just tr ->
            let
                ...
                vr =
                    List.sortBy (\a -> a.displayName) <| List.filter (\a -> a.selected) tr

                companyNames =
                    toHeadRow pText <| List.map .displayName vr

                rates =
                    rateUtil pt vr

                rateRow =
                    toBodyRow (pTextUtil pt) [ class "out-td" ] rates
                ...
                pdpRow =
                    toBodyRow "Drug Plan Monthly Premium" [ class "out-td" ] <| List.map (\a -> safeString pdpString) vr

                insuranceTotal =
                    List.map (\r -> currencyAddTwo pdp (safeCurrencyFloat (Just r))) rates

                insuranceTotalRow =
                    simpleTotalRow "Insurance Monthly Total" [ class "out-td" ] insuranceTotal

                partb =
                    safeCurrencyFloat model.partB

                partBRow =
                    toBodyRow "Part B Monthly Premium" [ class "out-td" ] <| List.map (\a -> safeString model.partB) vr

                grandTotal =
                    List.map (\t -> currencyAddThree pdp partb (safeCurrencyFloat (Just t))) rates

                grandTotalRow =
                    simpleTotalRow "Grand Monthly Total" [ class "out-td" ] grandTotal
...
viewRowsAll model =
    let
        showPreferred =
            viewRows model.viewPreferred Preferred model.tableRows

        showNonPreferred =
            viewRows model.viewNonpreferred NonPreferred model.tableRows

        showOutside =
            viewRows model.viewOutside Outside model.tableRows
    in
...
config =
    Table.customConfig
        ...
        , columns =
            ...
            , Table.intColumn "NAIC Code" .naic
            , categoryColumn
            ]
        , customizations =
...
toHeadRow rowname l =
    let
        ...
        lsh =
            [ th [ class "out-th", style "font-size" "2.0rem" ] [ text rowname ] ]
...
totalRow rowname col1 col2 l =
    let
        ...
        mm =
            List.minimum <| List.filter (\a -> a /= "$ ---.--") l
...
safedateloc dtt dts =
    case dtt of
        Just dt ->
            let
                pds =
                    List.indexedMap Tuple.pair dts

                flt =
                    List.filter (sdlhelp dt) pds
    ...
    if model.valid then
        let
            ...
            url2 =
                url1
                    ++ "zip="
                    ++ safeString model.zip.value
                    ++ "&age="
                    ++ safeString model.age.value
                    ++ "&county="
                    ++ strCounty model.county
                    ++ "&gender="
                    ++ genderString model.gender
                    ++ "&tobacco="
                    ++ (model.tobacco |> boolString)
                    ++ "&discounts="
                    ++ (model.discounts |> boolString)
                    ++ "&date="
                    ++ strMaybeDate model.date

            url3 =
                checkAddPlan model.planN "N" url2

            url4 =
                checkAddPlan model.planF "F" url3

            url5 =
                checkAddPlan model.planG "G" url4
        in
        Http.get
            { url = url5
...
getPDP model =
    let
        ...
        year1 =
            String.fromInt model.pdpYear1

        year2 =
            String.fromInt model.pdpYear2

        base_url =
            "https://medicare-school-quote-tool.herokuapp.com/api/pdp?"
    in
    Http.get
...
findDisplayNameUtil i tpls =
    let
        ls =
            List.filter (\a -> Tuple.first a == i) tpls
    in
