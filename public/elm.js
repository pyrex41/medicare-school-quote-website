...
var $author$project$Main$isValid = function (model) {
	...
	if (_v0.$ === 'Just') {
		...
		var newModel = _Utils_update(
			model,
			{
				valid: A3($elm$core$List$foldl, $elm$core$Basics$and, true, validList)
			});
		return newModel.valid;
	} else {
		return false;
	}
};
...
var $author$project$Main$formatColumn = F3(
	function (pdp, partb, ttr) {
		var fplan = $author$project$Main$safeCurrencyFloat(
			$elm$core$Maybe$Just(ttr.fRate));
		var fpdp = $author$project$Main$safeCurrencyFloat(pdp);
		var fpartb = $author$project$Main$safeCurrencyFloat(partb);
		var total = '$' + A2($myrho$elm_round$Round$round, 2, (fpdp + fpartb) + fplan);
		return A2(
			...
			_Utils_ap(
				...
				A2(
					...
					function (tx) {
						return A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$Background$color(
									A3($mdgriffith$elm_ui$Element$rgb, 0, 0.1, 0)),
									$mdgriffith$elm_ui$Element$Border$color(
									A3($mdgriffith$elm_ui$Element$rgb, 0, 0.9, 0))
								]),
					...
					_List_fromArray(
						[
							ttr.fRate,
							$author$project$Main$safeString(pdp),
							$author$project$Main$safeString(partb),
							total
						]))));
	});
	...
	if (_v0.$ === 'Just') {
		...
		return A2(
			...
			A2(
				...
				A2(
					$elm$core$List$map,
					A2($author$project$Main$formatColumn, model.pdpRate, model.partB),
					vr)));
	} else {
		return A2(
			$elm$html$Html$div,
